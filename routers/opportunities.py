...
from schemas import opportunity as opportunity_schemas, skill as skill_schemas
...
def create_opportunity(
    ...
    """
    # Check if user has faculty or industry_partner role
    ...
    db_opportunity = models.Opportunity(
        posted_by_user_id=current_user.id, # Ensure opportunity is linked to the creator
    ...
    db.commit()
    db.refresh(db_opportunity) # Refresh again to load the newly added skills

    # Eager load required skills for the response
...
def get_all_opportunities(
    ...
    if location:
        query = query.filter(models.Opportunity.location.ilike(f"%{location}%")) # Case-insensitive search
...
# You can add update and delete endpoints here, with appropriate authorization checks